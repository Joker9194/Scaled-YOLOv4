...
def train(hyp, opt, device, tb_writer=None):
    ...
    os.makedirs(wdir, exist_ok=True)  # make dir
    last = wdir + 'last.pt'  # last.pt
    best = wdir + 'best.pt'  # best.pt
    results_file = str(log_dir / 'results.txt')  # save the result to result.txt
    ...
    if pretrained:
        # torch_distributed_zero_first(rank): 用于同步不同进程对数据读取的上下文管理
        ...
        # 这里自己添加了 and not resume
        exclude = ['anchor'] if opt.cfg and not opt.resume else []  # exclude keys
    ...
    # 'P', 'R', 'mAP', 'mAP75', 'F1', 'val GIoU', 'val Objectness', 'val Classification'
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                # Forward pred: [3, N, 3(number anchor), input_size/stride, input_size/stride, xywh+obj+cls]
                pred = model(imgs)
                # pred = model(imgs.to(memory_format=torch.channels_last))

                # Loss loss_items=[lbox, lobj, lcls, loss]
        ...
        if rank in [-1, 0]:
            ...
            with open(results_file, 'a') as f:
                f.write(s + '%10.4g' * 8 % results + '\n')  # P, R, mAP, mAP75, F1, test_losses=(GIoU, obj, cls)
            ...
            # Update best mAP
            fi = fitness(np.array(results).reshape(1, -1))  # fitness_i = weighted combination of [P, R, mAP, mAP75, F1]
            if fi > best_fitness:
                best_fitness = fi
...
if __name__ == '__main__':
    ...
    data: 数据集配置文件，包括train、val、test、nc、names等
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            print('\nStart Tensorboard with "tensorboard --logdir %s", view at http://localhost:6006/' % opt.logdir)
            tb_writer = SummaryWriter(log_dir=increment_dir(Path(opt.logdir) / opt.name, exist_ok=opt.exist_ok))  # runs/exp
