...
def get_latest_run(search_dir='./runs'):
    ...
    :param search_dir: 要搜索的文件的根目录，默认是'./runs'
    :return: 路径列表中创建时间最近的last文件路径
    """
...
def labels_to_class_weights(labels, nc=80):
    """ 在train.py中使用，获得每个类别的权重，标签频率越高的类权重低
    Get class weights (inverse frequency) from training labels
    :param labels: 所有真实标签labels
    :param nc: 数据集的类别数
...
def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
    """ 在train.py中使用，利用labels_to_class_weights得到的每个类别的权重得到每一张图片的权重，再对图片进行按权重进行采样
    Produces image weights based on class mAPs
    :param labels: 所有真实标签labels
    :param nc: 数据集的类别数
    :param class_weights: size: [80], labels_to_class_weights得到的每个类别的权重
    :return: torch.from_numpy(weights): 每一个类别根据labels得到的占比(次数越多权重越小) tensor
    """
    n = len(labels)
...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    """
    在detect.py和test.py中使用
    Rescale coords (xyxy) from img1_shape to img0_shape
    :param img1_shape: coords所在的feature map的尺寸
    :param coords: box坐标信息[x1y1x2y2]
    :param img0_shape: 要映射到的图像尺寸
    :param ratio_pad: 缩放比例gain和pad值。None就先计算gain和pad值再pad+scale，不为空就直接pad+scale
    :return:
    """
    if ratio_pad is None:  # calculate from img0_shape
        # 先将要缩放图像的较长边缩放到相应尺寸，较短边同比例缩放
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = old / new
        # wh中有一个会为0，主要是pad另一个
        pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh padding
    else:
        gain = ratio_pad[0][0]  # 指定比例
        pad = ratio_pad[1]  # 指定pad
...
def compute_loss(p, targets, model):  # predictions, targets, model
    """
    :param p: 预测值，由Detect Head的输出
              tensor格式，list列表，存放三个tensor，对应三个Head的输出
              如: [N, 3, 80, 80, 85], [N, 3, 40, 40, 85], [N, 3, 20, 20, 85]
                  [bs, number anchor, height, width, xywh + obj + cls]
    :param targets: the ground truth after data augment, size: [target_number, img_index + class + xywh]
    :param model:
    :return: loss, loss_items=[lbox, lobj, lcls, loss]
    """
...
def increment_dir(path, exist_ok=True, sep=''):
    """
    递增路径 如 run/exp --> runs/exp{sep}0, runs/exp{sep}1 etc.
    @param path: 路径
    @param exist_ok: True
    @param sep: 文件名的后缀  默认''
    @return:
